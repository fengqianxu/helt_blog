...
test("keeps project assets and API-backed front-end routes in place", async () => {
  ...
  assert.match(shell, /className="moment-tags" aria-label="说说标签"/);
  assert.match(shell, /tag_ids: moment\.tags\.map\(\(tag\) => tag\.id\)/);
  assert.match(shell, /className="moment-editor-side"/);
  assert.doesNotMatch(shell, /与博客文章共用同一套标签/);
  assert.match(shell, /body: JSON\.stringify\(\{[\s\S]*?content: draft\.content,[\s\S]*?tag_ids: draft\.tag_ids/);
  assert.doesNotMatch(shell, /开发时的灵光、生活里的小事，以及值得记住的节点。|按时间倒序排列/);
  assert.match(shell, /draft && typeof document !== "undefined" && createPortal\(<div className="moment-editor-overlay"[\s\S]*?<\/div>, document\.body\)\}/);
  ...
  assert.match(styles, /\.moment-editor-overlay\s*\{[^}]*background:\s*rgba\(8,6,13,\.22\)[^}]*backdrop-filter:\s*blur\(16px\) saturate\(1\.12\)/);
  assert.match(styles, /\.moment-editor-overlay > section\s*\{[^}]*background:\s*linear-gradient[^}]*backdrop-filter:\s*blur\(32px\) saturate\(1\.28\)/);
  assert.match(styles, /\.moment-editor-content textarea\s*\{[^}]*background:\s*color-mix\(in srgb,var\(--bg\) 68%,transparent\)[^}]*backdrop-filter:\s*blur\(12px\)/);
