...
export function ReviewManager({ notify }: { notify: Notify }) {
  ...
  return <>
    <div className="admin-title review-title">
      ...
      <button className="admin-primary" type="button" onClick={() => section === "comments" ? window.open("/artalk/", "_blank", "noopener,noreferrer") : void loadFriends()}>
        {section === "comments" ? "打开评论控制台 ↗" : "刷新申请"}
      </button>
    </div>
    ...
    <nav className="review-section-switch" aria-label="审核内容">
      <button type="button" className={section === "comments" ? "active" : ""} aria-pressed={section === "comments"} onClick={() => setSection("comments")}>
        <span>◫</span><b>评论</b><small>Artalk 控制台</small>
      </button>
    ...
    {section === "comments" ? (
      <section className="admin-panel comment-admin-card">
        ...
        <div>
          ...
          <a className="review-inline-action" href="/artalk/" target="_blank" rel="noreferrer">进入评论审核 →</a>
        </div>
      </section>
    ) : (
